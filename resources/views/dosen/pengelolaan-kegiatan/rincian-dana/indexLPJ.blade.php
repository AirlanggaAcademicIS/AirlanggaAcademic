...
@foreach($kegiatan as $i => $kegiatan1)
 <a data-toggle="modal" data-target="#modalRincianDana{{$kegiatan1->id_kegiatan}}" class="btn btn-danger btn-xs">
          <i class="fa fa-trash-o"></i> Tambah Dana</a>       
   <!-- Modal Proposal-->
  <div class="modal fade" id="modalRincianDana{{$kegiatan1->id_kegiatan}}" role="dialog">
    <div class="modal-dialog">
    
      <!-- Modal content-->
   <form id="rincianDanaTabel" method="post" action="{{url('dosen/pengelolaan-kegiatan/rincian-dana/'.$kegiatan1->id_kegiatan.'/tambahDanaLPJ')}}" enctype="multipart/form-data"  class="form-horizontal">
...
<table id="rincianDanaTabel1" class="table table-striped table-bordered" cellspacing="0">
  ...
  <div class="modal fade" id="modalRincianDana{{$dana->id_rdana}}" role="dialog">
    <div class="modal-dialog">
      ...
      <!-- Modal content-->
   <form id="rincianDanaTabel" method="post" action="{{url('dosen/pengelolaan-kegiatan/rincian-dana/'.$dana->id_rdana.'/tambahDanaEditLPJ1')}}" enctype="multipart/form-data"  class="form-horizontal">
                   <input type="hidden" name="_token" value="{{ csrf_token() }}">
...
<a href="{{url('dosen/pengelolaan-kegiatan/daftarPengajuanKonfirmasiLPJ')}}" class="btn btn-success btn-xs"> Simpan</a>
@endforeach
