 ...
 <a data-toggle="modal" data-target="#modalStrukturPanitia{{$kegiatan->id_kegiatan}}" class="btn btn-danger btn-xs">
  ...
  <div class="modal fade" id="modalStrukturPanitia{{$kegiatan->id_kegiatan}}" role="dialog">
    <div class="modal-dialog">
      ...
      <!-- Modal content-->
    <form id="strukturPanitia" method="post" action="{{url('mahasiswa/pengelolaan-kegiatan/input-struktur-panitia/'.$kegiatan->id_kegiatan.'/tambahPanitia')}}" enctype="multipart/form-data"  class="form-horizontal">
      ...
      <div class="modal-content">
        ...
        <div class="modal-body" class="col-sm-12">
          <label for="mahasiswa" class="col-sm-2 control-label">Mahasiswa</label>
            ...
            <option value="{{ $data->nim }}"> {{ $data->biodata['nama_mhs'] }}</option>
...
<table id="struturPanitiaTabel1" class="table table-striped table-bordered" cellspacing="0">
  <thead>
    <tr>
  ...
  <tbody>
   @forelse($struktur as $i => $pan) 
    <tr>
      ...
      <td width="20%" style="text-align:center">{{$pan->mahasiswa['nama_mhs']}}</td>
      <td width="10%" style="text-align:center">{{$pan->jabatan['jabatan']}}</td>
      <td> <a data-toggle="modal" data-target="#modalStrukturPanitiaEdit{{$kegiatan->id_kegiatan}}{{$pan->nim_id}}" class="btn btn-danger btn-xs">
          <i class="fa fa-trash-o"></i> Edit Panitia</a>
          
     <!-- Modal Proposal-->
  <div class="modal fade" id="modalStrukturPanitiaEdit{{$kegiatan->id_kegiatan}}{{$pan->nim_id}}" role="dialog">
    <div class="modal-dialog">
    
      <!-- Modal content-->
   <form id="strukturPanitiaEdit" method="post" action="{{url('mahasiswa/pengelolaan-kegiatan/input-struktur-panitia/'.$kegiatan->id_kegiatan.'/tambahPanitia1')}}" enctype="multipart/form-data"  class="form-horizontal">
                   <input type="hidden" name="_token" value="{{ csrf_token() }}">
      ...
      <div class="modal-content">
        ...
        <div class="modal-body" class="col-sm-12">
          <label for="mahasiswa" class="col-sm-2 control-label">Mahasiswa</label>
             <select class="form-control input-sm" name="panitiaKegiatan" id="panitiaKegiatan" value="{{$pan->nim_id}}">
            
            <option value="{{ $pan->nim_id }}"> {{$pan->biodata['nama_mhs']}}</option>
...
<a href="{{url('mahasiswa/pengelolaan-kegiatan/rincian-rundown/'.$kegiatan->id_kegiatan)}}" class="btn btn-success btn-xs"> Simpan</a>
@endforeach
